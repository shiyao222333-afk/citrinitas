...
- PPStructureV3 依赖缺失导致回退到基础 PaddleOCR — 安装 `paddlex[ocr]` 额外依赖，PPStructureV3 现在正常工作（结构化识别：文字+表格+公式+图片区域）
- GPU 模式不可用（装的是 CPU 版 `paddlepaddle 3.0.0`）— 卸载 CPU 版，安装 GPU 版 `paddlepaddle-gpu 3.3.0`（CUDA 12.6），RTX 3080 现在被使用
